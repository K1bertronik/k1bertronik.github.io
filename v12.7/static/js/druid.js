...
function LightningBolt(magicalDamage){
    ...
    var lightningBoltBonusAlm = (parseFloat(document.getElementById('lightningBoltBonusAlm').value) || 0) / 100;

    for (var level = 0; level < 5; level++) {
        var damage = (baseValues[level] + magicalDamage * (percentageIncreases[level] / 100 + lightningBoltBonus + lightningBoltBonusAlm)) * (1 + relicBonus + unitedBonus);
...
function Tornado(magicalDamage){
    ...
    var tornadoBonus = (parseFloat(document.getElementById('tornadoBonusAlm').value) || 0) / 100;

    var windstormBonus = document.getElementById('windstormBonus').checked ? 0.15 : 0;

    for (var level = 0; level < 4; level++) {
        var damage = magicalDamage * (percentageIncreases[level] / 100 + tornadoBonus) * (1 + windstormBonus);
        damageLevels.push(damage);
    }
...
function InvigoratingStream(magicalDamage){
    ...
    var invigoStreamBonusAlm = (parseFloat(document.getElementById('invigoStreamBonusAlm').value) || 0) / 100;

    for (var level = 0; level < 4; level++) {
        var heal = magicalDamage * (percentageIncreases[level] / 100 + invigoStreamBonus + invigoStreamBonusAlm);
        healLevels.push(heal);
    }
