document.getElementById('calcForm').addEventListener('submit', function (event) {
    ...
    var blazingGroundDamage = calculateDamage(BlazingGround(magicalDamage), false, isPyromaniac, false, false);
...
function calculateDamage(skillDamageLevels, isInstantDamage=true, isPyromaniac=false, isTalent=false, isBasicSkill=true) {
    ...
    var excessEnergyBonus = document.getElementById('excessEnergyBonus').checked ? 0.14 : 0;
    var natureBonus = (isInstantDamage ? (document.getElementById('natureBonus').checked ? 0.10 : 0) : 0);

    var elementalPowerBonus = (parseFloat(document.getElementById('elementalPowerBonus').value) || 0) / 100;

    for (var level = 0; level < skillDamageLevels.length; level++) {
        var skillDamage = skillDamageLevels[level];
        var totalDamage = skillDamage * (1 - Math.max(0, targetMagicalDefence - (penetration + dotPenBonus + instPenBonus))) * (1 + talentDmgBonus + talentPVEDmgBonusI + talentPVEDmgBonusII + excessEnergyBonus + natureBonus) * (1 - targetResilience) * (1 + ferocity) * (1 + castleDmg + exclusiveAttackBonus) * (1 + elementalPowerBonus);
        if (isPyromaniac) {
...
function FireBall(magicalDamage){
    ...
    var fireBallBonusAlm = (parseFloat(document.getElementById('fireBallBonusAlm').value) || 0) / 100;
...
function ShatteredStone(magicalDamage){
    ...
    var percentageIncreases = [125.0, 135.0, 145.0, 160.0, 175.0];
...
function Frostbolt(magicalDamage){
    ...
    var frostboltBonus = document.getElementById('frostboltBonus').checked ? 0.075 : 0;
...
function Overload(magicalDamage){
    ...
    var overloadBonusAlm = (parseFloat(document.getElementById('overloadBonusAlm').value) || 0) / 100;

    for (var level = 0; level < 4; level++) {
		var damage = magicalDamage * (percentageIncreases[level] / 100 + overloadBonus + overloadBonusAlm);
		damageLevels.push(damage);
	}
...
function RoaringFlame(magicalDamage){
    ...
    var roaringFlameBonusAlm = (parseFloat(document.getElementById('roaringFlameBonusAlm').value) || 0) / 100;
