document.getElementById('calcForm').addEventListener('submit', function (event) {
	...
	var plaguedMinionExplodeDamage = calculateDamage(PlaguedMinionExplode(physicalDamage), 'physical', true, false);
    var plaguedMinionDamage = calculateDamage(PlaguedMinion(magicalDamage), 'magical', true, false);
    ...
    updateDamageValues(plaguedMinionExplodeDamage, "plaguedMinionExplodeRow");
...
function calculateDamage(skillDamageLevels, damageType, isTalent=false, isBasicSkill=true) {
    ...
    var devotionBonus = (parseFloat(document.getElementById('devotionBonus').value) || 0) / 100;

    for (var level = 0; level < skillDamageLevels.length; level++) {
        var skillDamage = skillDamageLevels[level];
        var totalDamage = skillDamage * (damageType === 'physical' ? (1 - targetPhysicalReduction) : (1 - targetMagicalReduction)) * (1 + talentPVEDmgBonusI + talentPVEDmgBonusII) * (1 - targetResilience) * (1 + ferocity) * (1 + castleDmg + exclusiveAttackBonus + devotionBonus);
...
function DarkPrism(physicalDamage, magicalDamage){
    ...
    var percentagePhysIncreases = [105.0, 110.0, 115.0, 120.0, 125.0];
    var percentageMagIncreases = [120.0, 125.0, 130.0, 135.0, 140.0];
    ...
    var darkPrismBonusAlm = document.getElementById('darkPrismBonusAlm').value;

    darkPrismBonusAlm = darkPrismBonusAlm.split(',');

    var physDarkPrismBonusAlm = parseFloat(darkPrismBonusAlm[0]) / 100;
    var magDarkPrismBonusAlm = parseFloat(darkPrismBonusAlm[1]) / 100;
    ...
    for (var level = 0; level < 5; level++) {
        if (physicalDamage > magicalDamage) {
            ...
            var damage = (baseMagValues[level] + magicalDamage * (percentageMagIncreases[level] / 100 + darkPrismBonus + magDarkPrismBonusAlm)) * (1 + relicBonus + unitedBonus);
        }
...
function Call(physicalDamage){
	...
	var callBonusAlm = (parseFloat(document.getElementById('callBonusAlm').value) || 0) / 100;
    ...
    for (var level = 0; level < 5; level++) {
        var damage = (physicalDamage * (percentageIncreases[level] / 100 + callBonus + callBonusAlm)) * (1 + relicBonus) * (1 + summonerSkillBonus);
        damageLevels.push(damage);
	}
...
function Oppression(magicalDamage){
    ...
    var percentageIncreases = [65.0, 70.0, 75.0, 85.0, 100.0];
...
function OtherworldFire(magicalDamage, isEmbrace){
    ...
    var percentageIncreases = [60.0, 65.0, 75.0, 90.0];
	
	var embraceBonus = isEmbrace ? 1 : 0;
...
function PlaguedMinionExplode(physicalDamage){
    var damageLevels = [];
	
    var percentagePhysIncrease = 0.45;

    var damage = physicalDamage * percentagePhysIncrease;
    damageLevels.push(damage);
		
    return damageLevels;
}

function PlaguedMinion(magicalDamage){
    var damageLevels = [];
	
    var percentageMagIncrease = 0.25;
	
    var damage = magicalDamage * percentageMagIncrease;
    damageLevels.push(damage);
