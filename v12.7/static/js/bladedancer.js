...
function VortexBlade(physicalDamage) {
    ...
    var percentageBoostIncreases = (isPVPTarget ? [6.0, 9.0, 12.0, 16.0, 20.0] : [15.0, 20.0, 25.0, 30.0, 40.0]);

    var relicBonus = document.getElementById('relicBonus').checked ? 0.12 : 0;

    var vortexStrikeBonus = (parseFloat(document.getElementById('vortexStrikeBonus').value) || 0) / 100;
    var vortexStrikeBonusAlm = (isPVPTarget ? 0 : (parseFloat(document.getElementById('vortexStrikeBonusAlm').value) || 0) / 100)

    for (var level = 0; level < 5; level++) {
        var damage = physicalDamage * (percentageIncreases[level] / 100 + vortexStrikeBonus) * (1 + relicBonus);
        var boostedDamage = damage * (1 + percentageBoostIncreases[level] / 100 + vortexStrikeBonusAlm);
...
function MagicTransformation(physicalDamage, maxHealth, currentHealth){
    ...
    var bastionBonus = document.getElementById('bastionBonus').checked ? 0.1 : 0;
    ...
    for (var level = 0; level < 4; level++) {
        ...
        var shield = (baseshield * (1 + boostPerHealth / 100)) + (maxHealth * bastionBonus);
...
function SonicBoom(physicalDamage){
    ...
    var sonicBoomBonusAlm = (parseFloat(document.getElementById('sonicBoomBonusAlm').value) || 0) / 100;

    for (var level = 0; level < 4; level++) {
        var damage = physicalDamage * (percentageIncreases[level] / 100 + sonicBoomBonus + sonicBoomBonusAlm);
        damageLevels.push(damage);
    }
...
function StrikeHurricane(physicalDamage){
    ...
    var strikeHurricaneBonus = (parseFloat(document.getElementById('strikeHurricaneBonusAlm').value) || 0) / 10

    for (var level = 0; level < 4; level++) {
        var damage = countOfHits[level] * (physicalDamage * (percentageIncreases[level] / 100 + strikeHurricaneBonus));
        damageLevels.push(damage);
    }
