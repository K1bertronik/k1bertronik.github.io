...
function calculateDamage(skillDamageLevels, isInstantDamage=true, isTalent=false, isBasicSkill=true) {
    ...
    var isFirstStrike = document.getElementById('firstStrikeBonus').checked;

    var firstStrikeBonus = (isFirstStrike && isInstantDamage) ? (0.008 * ((targetCurrentHealth / targetMaxHealth) * 10)) : 0;

    var firstStrikePenBonus = (targetMaxHealth === targetCurrentHealth && isFirstStrike) ? 0.05 : 0;

    for (var level = 0; level < skillDamageLevels.length; level++) {
        var skillDamage = skillDamageLevels[level];
        var totalDamage = skillDamage * (1 - Math.max(0, targetPhysicalDefence - (penetration + dotPenBonus + instPenBonus + firstStrikePenBonus))) * (1 + talentDmgBonus + talentPVEDmgBonusI + talentPVEDmgBonusII) * (1 - targetResilience) * (1 + ferocity) * (1 + castleDmg + exclusiveAttackBonus) * (1 + huntersMarkBonus) * (1 + firstStrikeBonus);
...
function SappingShot(physicalDamage){
    ...
    doubleProfitBonus = (isPVPTarget ? doubleProfitBonus * 0.5 : doubleProfitBonus * 0.7);
...
function PathfindersShot(physicalDamage){
    ...
    var pathfindersArrowBonusAlm = (parseFloat(document.getElementById('pathfindersArrowBonusAlm').value) || 0) / 100;

    for (var level = 0; level < 4; level++) {
        var damage = physicalDamage * (percentageIncreases[level] / 100 + pathfindersArrowBonus + pathfindersArrowBonusAlm);
        damageLevels.push(damage);
    }
...
function ForestTrap(physicalDamage){
    ...
    var percentageIncreases = [75.0, 85.0, 95.0, 110.0];
