...
function Splitting(physicalDamage){
    ...
    var splittingBonusAlm = (parseFloat(document.getElementById('splittingBonusAlm').value) || 0) / 100;

    for (var level = 0; level < 5; level++) {
        var damage = (baseValues[level] + physicalDamage * (percentageIncreases[level] / 100 + splittingBonus + splittingBonusAlm)) * (1 + relicBonus + unitedBonus);
...
function DangerousBlowDot(physicalDamage, isCrit=false){
    ...
    var dangerousBlowBonusAlm = (parseFloat(document.getElementById('dangerousBlowBonusAlm').value) || 0) / 100;

    for (var level = 0; level < 4; level++) {
        var damage = physicalDamage * (percentageIncreases[level] / 100 + dangerousBlowBonus + dangerousBlowBonusAlm) * (1 + (isCrit ? (percentageCritIncreases[level] / 100) : 0));
        damageLevels.push(damage);
    }
...
function SplittingBlow(physicalDamage){
    ...
    var splittingBlowBonusAlm = (parseFloat(document.getElementById('splittingBlowBonusAlm').value) || 0) / 100;

    for (var level = 0; level < 4; level++) {
        var damage = physicalDamage * (percentageIncreases[level] / 100 + splittingBlowBonus + splittingBlowBonusII + splittingBlowBonusAlm);
        damageLevels.push(damage);
    }
...
function Bloodthirst(physicalDamage){
    ...
    var percentageIncreases = [160.0, 180.0, 200.0, 225.0];
