...
function calculateDamage(skillDamageLevels, isInstantDamage=true, isTalent=false, isBasicSkill=false) {
    ...
    var isSniperBonus = document.getElementById('sniperBonus').checked;
    var sniperBonus = isSniperBonus ? (parseFloat(document.getElementById('rangeOfSkill').value) || 0) * 0.05 : 0;

    for (var level = 0; level < skillDamageLevels.length; level++) {
        var skillDamage = skillDamageLevels[level];
        var totalDamage = skillDamage * (1 - Math.max(0, targetPhysicalDefence - (penetration + dotPenBonus + instPenBonus))) * (1 + talentDmgBonus + talentPVEDmgBonusI + talentPVEDmgBonusII + (isHeavyArtillery ? (2 * (attackSpeed / 3)) : 0)) * (1 - targetResilience) * (1 + ferocity) * (1 + castleDmg + exclusiveAttackBonus) * (1 + sniperBonus);
...
function PowerfulShot(physicalDamage){
    ...
    var powerfulShotBonusAlm = (parseFloat(document.getElementById('powerfulShotBonusAlm').value) || 0) / 100;

    var isBullseye = document.getElementById('isBullseye').checked;
    var distance = (parseFloat(document.getElementById('rangeOfShot').value) || 0);
    ...
    var currentHealthBoost = ((currentHealth / maxHealth) * 10) * 0.015;
    var distanceBoost = distance * 0.015;

    for (var level = 0; level < 5; level++) {
        var damage = (baseValues[level] + physicalDamage * (percentageIncreases[level] / 100 + powerfulShotBonus + powerfulShotBonusAlm)) * (1 + relicBonus + unitedBonus) * (1 + (isBullseye ? (currentHealthBoost + distanceBoost): 0));
...
function RangersBlessing(physicalDamage){
    ...
    var blessingBonusAlm = (parseFloat(document.getElementById('blessingBonusAlm').value) || 0) / 100;
    ...
    for (var level = 0; level < 5; level++) {
        var damage = physicalDamage * (percentageIncreases[level] / 100 + blessingBonus + blessingBonusAlm) * (isflexibleBowstring ? 0.3 : 1) * (isflexibleBowstring ? (1 + attackStrength) : 1);
        damageLevels.push(damage);
    }
...
function ExplosiveTrap(physicalDamage){
    ...
    var percentageIncreases = [115.0, 130.0, 150.0, 170.0];
...
function HailOfArrows(physicalDamage){
    ...
    var hailOfArrowsBonus = document.getElementById('hailOfArrowsBonus').checked ? 0.09 : 0;
    var hailOfArrowsBonusAlm = (parseFloat(document.getElementById('hailOfArrowsBonusAlm').value) || 0) / 100;

    var isFireVoley = document.getElementById('isFireVoley').checked;

    var fireVoleyBonus = isFireVoley ? (isPVPTarget ? 0.06 : 0.08) : 0;

    var distance = (parseFloat(document.getElementById('rangeOfHail').value) || 0);

    for (var level = 0; level < 4; level++) {
        var damage = physicalDamage * (percentageIncreases[level] / 100 + hailOfArrowsBonus + hailOfArrowsBonusAlm) * (1 + (isFireVoley ? (fireVoleyBonus * distance) : 0));
        damageLevels.push(damage);
    }
...
function VengefulShot(physicalDamage){
    ...
    var vengefulshotBonusAlm = (parseFloat(document.getElementById('vengefulshotBonusAlm').value) || 0) / 100;

    for (var level = 0; level < 4; level++) {
        var damage = physicalDamage * (percentageIncreases[level] / 100 + vengefulshotBonusAlm);
        damageLevels.push(damage);
    }
