document.getElementById('calcForm').addEventListener('submit', function (event) {
    ...
    var stingBeeDamage = calculateDamage(StingBee(physicalDamage));
    ...
    updateDamageValues(stingBeeDamage, "stingBeeRow");
});
...
function calculateDamage(skillDamageLevels, isInstantDamage=true, isBasicSkill=false) {
    ...
    var ruthlessnessScale = parseFloat(document.getElementById('ruthlessnessScale').value) || 0;

    var ruthlessnessBonus = (parseFloat(document.getElementById('ruthlessnessBonusAlm').value) || 0);

    var ruthlessnessBoost = 0;
    if (targetMaxHealth > 0 && targetCurrentHealth > 0 && ruthlessnessScale !== 0) {
        ruthlessnessBoost = (((targetMaxHealth - targetCurrentHealth) / targetMaxHealth * 100) / ruthlessnessScale) * (1 + ruthlessnessBonus);
    }

    for (var level = 0; level < skillDamageLevels.length; level++) {
        var skillDamage = skillDamageLevels[level];
        var totalDamage = skillDamage * (1 - Math.max(0, targetPhysicalDefence - (penetration + dotPenBonus + instPenBonus))) * (1 + talentDmgBonus + talentPVEDmgBonusI + talentPVEDmgBonusII + bladeEchoBonus + seriesBonus) * (1 - targetResilience) * (1 + ferocity) * (1 + castleDmg + exclusiveAttackBonus) * (1 + ruthlessnessBoost / 100);
...
function MercilessStrike(physicalDamage){
    ...
    var strikeBonusAlm = (parseFloat(document.getElementById('strikeBonusAlm').value) || 0) / 100;

    for (var level = 0; level < 5; level++) {
        var damage = (baseValues[level] + physicalDamage * (percentageIncreases[level] / 100 + strikeBonus + strikeBonusAlm)) * (1 + relicBonus + unitedBonus);
...
function ElusiveJump(physicalDamage){
    ...
    var jumpBonusAlm = (parseFloat(document.getElementById('jumpBonusAlm').value) || 0) / 100;

    for (var level = 0; level < 4; level++) {
        var damage = physicalDamage * (percentageIncreases[level] / 100 + jumpBonus + jumpBonusAlm);
        damageLevels.push(damage);
    }
...
function PoisonousBlades(physicalDamage){
    ...
    var poisonBonusAlm = (parseFloat(document.getElementById('poisonBonusAlm').value) || 0) / 100;

    for (var level = 0; level < 4; level++) {
        var damage = physicalDamage * (percentageIncreases[level] / 100 + poisonBonusAlm);
        damageLevels.push(damage);
    }
...
function Ricochet(physicalDamage){
    ...
    var ricochetBonusAlm = (parseFloat(document.getElementById('ricochetBonusAlm').value) || 0) / 100;
    
    for (var level = 0; level < 4; level++) {
        var damage = physicalDamage * (percentageIncreases[level] / 100 + ricochetBonus + ricochetBonusAlm);
        damageLevels.push(damage);
    }
...
function StingBee(physicalDamage){
    var damageLevels = [];
