document.getElementById('calcForm').addEventListener('submit', function (event) {
    ...
    var ancestorsHandShield = AncestorsHand(magicalDamage)[0];
    var ancestorsHandOnSelfShield = AncestorsHand(magicalDamage)[1];
    var healingTotemHeal = calculateHeal(HealingTotem(magicalDamage));
    ...
    updateDamageValues(ancestorsHandOnSelfShield, "ancestorsHandOnSelfRow");
...
function LightningBall(magicalDamage){
    ...
    var lightningBallBonusAlm = (parseFloat(document.getElementById('lightningBallBonusAlm').value) || 0) / 100;

    for (var level = 0; level < 5; level++) {
        var damage = (baseValues[level] + magicalDamage * (percentageIncreases[level] / 100 + lightningBallBonus + lightningBallBonusAlm)) * (1 + relicBonus + unitedBonus);
...
function FireTotemTick(magicalDamage){
    ...
    var fireTotemBonus = (parseFloat(document.getElementById('fireTotemBonus').value) || 0) / 100;
    var fireTotemBonusAlm = (parseFloat(document.getElementById('fireTotemBonusAlm').value) || 0) / 100;

    for (var level = 0; level < 4; level++) {
        var damage = magicalDamage * (percentageIncreases[level] / 100 + fireTotemBonus + fireTotemBonusAlm);
        damageLevels.push(damage);
    }
...
function AncestorsHand(magicalDamage){
    var shieldLevels = [];
    var selfShieldLevels = [];

    var percentageIncreases = [130.0, 150.0, 170.0, 200.0];
    var selfPercentageIncreases = [90.0, 105.0, 120.0, 140.0];

    for (var level = 0; level < 4; level++) {
        var shield = magicalDamage * (percentageIncreases[level] / 100);
        var selfShield = magicalDamage * (selfPercentageIncreases[level] / 100);

        shieldLevels.push(shield);
        selfShieldLevels.push(selfShield);
    }

    return [shieldLevels, selfShieldLevels];
}


function HealingTotem(magicalDamage){
    var healLevels = [];
    ...
    for (var level = 0; level < 4; level++) {
        var heal = magicalDamage * (percentageIncreases[level] / 100 + healTotemBonus);
        healLevels.push(heal);
    }
